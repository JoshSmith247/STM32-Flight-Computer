//! Sensor health / plausibility monitor.
//!
//! Reads the raw sensor state at ~20 Hz, applies physical plausibility checks, and
//! publishes per-sensor health flags to `STATE.sensor_health`. These gate the pre-arm
//! checks in `arming_task` so the craft won't arm on a dead, clipping, or implausible
//! sensor. It does NOT filter or correct the data — it only flags it.
//!
//! The IMU/baro checks are intended as *pre-arm* gates (evaluated at rest on the
//! ground). They are deliberately wide so normal flight transients don't trip them;
//! using them as in-flight failsafes would want hysteresis, which is out of scope here.
...
// IMU plausibility. Accel magnitude must sit in a sane band around 1 g — below ACCEL_MIN
// the sensor is dead/zeroed, above ACCEL_MAX it is clipping or being shaken hard. Gyro
// must not be pinned near full-scale (±2000 dps).
const G: f32 = 9.80665;
...
pub async fn health_task() {
    ...
    loop {
        ...
        // ── Mag: valid is set true by mag_task only after calibration completes ──
        let mag_ok = STATE.mag_data.lock().await.valid;

        // ── GPS: 3-D fix with bounded horizontal accuracy ───────────────────────
        let gps = *STATE.gps_fix.lock().await;
        let gps_ok = gps.fix_ok
            && gps.fix_type >= 3
